...
async function fetchModels(key){
  ...
  else if(type==="gemini") url="https://api.gemini.com/v1/models";

  const res=await fetch(url,{headers:{"Authorization":`Bearer ${key}`}});
  if(!res.ok) throw new Error(`${type} 서버 모델 불러오기 실패`);
  const data=await res.json();
  // 모델 목록 반환 (Groq/OpenAI/Gemini 공통 구조 맞춤 필요)
...
async function callAI(scene){
  ...
  else if(type==="gemini") url=`https://api.gemini.com/v1/models/${modelId}/completions`;
